...
def getTeammates(team,DriversList):
    for i in DriversList:
        if i == team:
            teammates = DriversList[i]
    return teammates

def getReleventRaces(team,raceList,driver):
    ValidRounds = {"driver":driver}
    for i in raceList:
        if i["Results"][0]["Constructor"]["constructorId"] == team:
            ValidRounds[int(i["round"])] = i["Results"][0]["position"]
    return ValidRounds
